...
let index = 1;
let autoTimer;
let previewArrow = document.querySelector(".arrow_left");
...
/*Code Here*/
function setClassOn() { /*设置当前图片对应按钮的显示*/
    for (let j = 0; j < buttons.length; j++) {
        buttons[j].removeAttribute("class");
    }
    onButton = document.getElementById("bt" + index);
    onButton.className = "on";
}
...
for (let j = 0; j < buttons.length; j++) {
    buttons[j].onclick = function () {
        clearInterval(autoTimer);
        index = this.getAttribute("id").charAt(2);
        wrap.style.left = index * -600 + "px";
        setClassOn();
        autoPlay();
    }
